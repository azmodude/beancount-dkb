...
class ECImporter(importer.ImporterProtocol):
    ...
    def __init__(
        ...
        self._expected_header = '"Kontonummer:";"{} / Girokonto";'.format(self.iban)
        ...
        with change_locale(locale.LC_NUMERIC, self.numeric_locale):
            with open(file_.name, encoding=self.file_encoding) as fd:
                ...
                for index, line in enumerate(reader):
                    ...
                    amount = Amount(
                        locale.atof(line["Betrag (EUR)"], Decimal), self.currency
                    )
                    date = datetime.strptime(line["Buchungstag"], "%d.%m.%Y").date()

                    if line["Verwendungszweck"] == "Tagessaldo":
                        entries.append(
                            data.Balance(meta, date, self.account, amount, None, None)
                        )
                    else:
                        payee = "{} {}".format(
